...
// transaction::transaction() {}  // empty transaction, not yet ready
...
transaction::transaction(std::shared_ptr<transaction_impl> impl)
    : _impl(impl) {}
...
proton::tracker transaction_impl::send_ctrl(proton::symbol descriptor, proton::value _value) {
    ...
    delivery.transaction(
        transaction(std::make_shared<transaction_impl>(*this)));
    std::cout
...
transaction transaction::mk_transaction_impl(sender &s, transaction_handler &h,
    ...
    std::shared_ptr<transaction_impl> _impl =
        std::make_shared<transaction_impl>(s, h, f);
    return transaction(_impl);
}
}
